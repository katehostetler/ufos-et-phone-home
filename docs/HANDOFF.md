...
- **FYI / not fixed:** `nasa-uap-d3a` ("Gemini 7 Audio Excerpt", `mediaType:"vid"`) and `nasa-uap-d3` ("Gemini 7 Transcript", `mediaType:"pdf"`) both sit at "Low Earth Orbit" → two pushpins (one red, one gold) perfectly overlapping, so it can look like "the LEO doc pin is red". Same overlap exists at any multi-record location (e.g. Hormuz ×5). Fix would be fanning out co-located pins, or deduping `points` by location with a representative colour — deferred.
