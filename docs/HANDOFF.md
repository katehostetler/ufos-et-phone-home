...
- **Pins: small beads + `points` model** (`pushpin.ts` + `GlobeApp.tsx` `points` useMemo). `PUSHPIN.beadRadius` 1.25→0.72 (regional 1.6→0.92, unused now), `altitude` 0.06→0.04, `needleRadius` 0.14→0.1, `touchScale` 1.5→1.3. The clickable hit-volume (`pointRadius`) is decoupled: `PUSHPIN.beadRadius * (isTouch ? 9 : 3.5)` — much bigger than the bead, so taps land. `points` builds one entry per **pin** (not per record, not strictly per location):
  - **precise location** (one lat/lng — "Greece", "Mexico", country points, etc.): collapse all its records to ONE pin, coloured by the dominant media type (ties → `TYPE_PRIORITY` pdf>img>vid). Fixes the old "stacked pins z-fight, wrong colour on top" bug (e.g. "LEO pin is red" was `nasa-uap-d3a` audio-`vid` on top of the transcript).
  - **regional location** (`location.regional` true — "Western United States", "Arabian Gulf", "Mediterranean Sea", …, with >1 record): SCATTER — one small pin per record, sunflower-disk distributed (`frac = sqrt((i+0.5)/n)`, `R = min(2 + sqrt(n), 7)°`, lng scaled by `1/cos(lat)`) around the region's lat/lng, each coloured by its OWN `mediaType`. (Disk + small pins → reads as "markers all over the area", not the giant gold *ring* the first fan-out attempt drew.)
  - Each entry carries `_pinKey` (record id if scattered, else location name — `customThreeObject` keys `pinMeshesRef` by it), `_pinLat/_pinLng` (the on-globe spot — `pointLat/pointLng/customThreeObjectUpdate` use these), `_pinType` (colour + tooltip glyph/class), `_count` (records at the location — drives "· +N more here" in `pointLabel`), `_ring`/`_ringRadius` (pulse: collapsed pin pulses if `counts.vid>0 || n>=3` with `ringRadius = min(2 + n*0.45, 7.5)`; scattered video pins pulse small `2.2`). `rings = points.filter(p => p._ring)`. `highlightPin(rec|null)` looks up `pinMeshesRef.get(rec.id) ?? get(rec.location.name)`. `openLocationModal` unchanged → clicking any pin (incl. a scattered one) gathers ALL records at that `location.name`. Counts: Western US 25, Arabian Gulf 13, Syria 12 (non-regional → 1 collapsed pin w/ a 7.4° pulse), Iraq 8 (same), United States 5, Mediterranean Sea 4.
