...
- **Lunar pins always face the camera** (`LunarMoon.tsx` `frame()`) — the 8 Apollo pins cluster around `CLUSTER_DIR` on one face of the Moon, and the Moon mesh never spun, so it could orbit to a spot where that face pointed away and the pins vanished. Fix: right after `group.position.set(...)`, get `cam.getWorldPosition(_camWorld)` + `moonRef.getWorldPosition(_moonWorld)`, `_dirToCam = (cam − moon).normalize()`, then `moonRef.quaternion.setFromUnitVectors(CLUSTER_DIR, _dirToCam)` + `moonRef.rotateX(8°)` (so it's slightly off head-on, reads as 3D). The group only carries position (no rotation) so parent-local == world space → no extra transform needed. Scratch `Vector3`/`Quaternion` allocated once, reused. Works under reduced-motion too (`a` is frozen at 0.7 → Moon sits at a fixed orbit point but still faces the camera from there). The `moonSaucerPivot` is a child of `moonRef`, so re-orienting the Moon carries the saucer along — its lap/spin/bob is unchanged (verified). Verified numerically: `clusterDirWorld · dirToCamera = 0.991` (≈cos 8°) at the live orbit position AND with the Moon teleported to the opposite octant; all 8 pin groups on the camera-facing hemisphere; screenshots confirm.
