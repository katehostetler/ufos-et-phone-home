...
- **Moon** — clicking it (`LunarMoon onSelect` → GlobeApp) sets `controlsRef.current.autoRotate = false`; `closeModalPreservingView` sets it back true. RecordModal shows "THE MOON" when `records.every(r => r.location?.name === "Moon")`.
...
- **Co-located pins fanned out** (`GlobeApp.tsx` `points` useMemo) — records sharing a `location.name` are grouped, and each gets a small ring offset (`_pinLat`/`_pinLng`, `radius = 0.5 + 0.2*(n-1)` degrees, `lng` scaled by `1/cos(lat)`). `pointLat`/`pointLng`/`customThreeObjectUpdate`/rings/`onQueueActiveChange`-fly all use `_pinLat/_pinLng`; click-grouping (`openLocationModal`) still keys off `location.name` so it gathers ALL records at the place. (This fixed the "LEO document pin shows red" — it was the `nasa-uap-d3a` audio-as-`vid` pin perfectly stacked on the `nasa-uap-d3` transcript pin.)
